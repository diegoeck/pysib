...
The C compiler and LAPACK are required to build the extension modules
for the nonlinear estimators (OE, ARMAX, BJ) and their filtered
variants.  On macOS, the Accelerate framework is available by default.
On Linux (Debian/Ubuntu), install the dependencies with:
...
A working C compiler and LAPACK must be available before running this
command.
...
\subsection{Running the driver programs}

The \texttt{Python/drivers/} directory contains one driver script per
numerical result presented in the article.  Each driver is
self-contained and requires no interactive input.  To run all drivers:
...
Expected outputs are stored in \texttt{Python/drivers/expected/}.
...
\paragraph{Filter sequence for \texttt{oe\_filtered}.}
...
The initial guess for stage~1 is an ARX estimate computed on the
most-narrowly filtered data; each subsequent stage warm-starts
from the previous estimate.

\paragraph{Filter sequence for \texttt{armax\_filtered} and
\texttt{bj\_filtered}.}
A first-order Butterworth low-pass filter is used at each stage:
\[
  L_i = \mathrm{Butterworth}\!\left(1,\; \omega_c = 0.1\,i\,\pi\right),
  \quad i = 1,\ldots,9,
\]
where $\omega_c$ is the $-3\,\mathrm{dB}$ cutoff in rad/sample.
The cutoff increases from $0.1\pi$ to $0.9\pi$, covering
progressively wider bands.
The initial guess is an ARX estimate on the unfiltered data; each
subsequent stage warm-starts from the previous estimate.
