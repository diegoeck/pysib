...
SISO polynomial system identification---an ARX-based initialization, a
smoothed-gradient phase, an incremental Gauss--Newton refinement, and filtered
continuation interpreted as cost-function shaping.  This strategy produced the
...
with an optimization procedure designed around the structure of the recursive
PEM criterion, which is the part of the toolbox examined in most detail below.
...
descent safeguards, curvature information, and failure handling.  The aim is not
a new global-convergence guarantee, but a reliable and reproducible local-search
strategy designed around the structure of the recursive PEM criterion for the
SISO polynomial models considered by the toolbox \citep{goodwin2003}.
...
so a single cost-based safeguard---rejecting and backtracking any trial point
that does not decrease the cost---handles both failure modes at once.  The price
...
toolbox its name: SIB (Slow Is Better), of which \texttt{pysib} is the Python
implementation.
...
Levenberg--Marquardt iteration applied to the recursive PEM criterion was found
...
\begin{equation}
  ...
  \qquad \beta_i \approx \frac{i}{1000},
...
Gauss--Newton ramp, and the iteration budgets are not tuned per problem.  This is
a deliberate design choice---the strategy favours robustness and determinism over
per-problem tuning---and, because the experiment scripts and the random seed are
provided, the absence of tuning is auditable rather than merely asserted.
...
preliminary experiments not reported here suggested that it is effective for them
as well, and further examples are straightforward to produce within the same
framework but are omitted here for space.
...
program and solves it with an interior-point optimizer, whereas \texttt{pysib}
...
around the recursive PEM criterion.  The noise level
