...
nonconvex example comparing standard OE with filtered continuation.
\end{abstract}
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    \texttt{*\_filtered} & Filtered continuation & PEM initialization \\
...
Formally, given a model \(m\) returned by any estimator, the one-step-ahead
prediction computed by \texttt{predict} is
\begin{equation}
  \hat y_p(t) =
  H^{-1}(q^{-1})G(q^{-1})u(t) + \bigl[1-H^{-1}(q^{-1})\bigr]y(t),
  \label{eq:predict-routine}
\end{equation}
where \(G\) and \(H\) are evaluated from the polynomials in \(m\) and the input
delay is included in \(B\) according to the storage convention.  The noise-free
simulation computed by \texttt{simulate} uses only the plant model,
\begin{equation}
  \hat y_s(t) = G(q^{-1})u(t).
...
using noisy data generated from \eqref{eq:experiment-plant}.  The noise level
...
error.  The Monte Carlo study uses \(M=100\) independent data records of length
...
summarize the simulation errors.  In this
benchmark, the two \texttt{pysib} estimates are more concentrated around the
true coefficients than the SIPPY OE estimates obtained in this configuration.
The histogram shows the same effect
from the model-output perspective: the \texttt{pysib} error distributions are
tighter and closer to zero.  The mean simulation error is also smaller for
\texttt{pysib} SM and OE.  All methods satisfy the success criterion in all
Monte Carlo runs, so the main distinction in this moderate-noise case is the
concentration of the estimates and the distribution of \(E_{\mathrm{sim}}\), not
...
other Python identification packages and with the established MATLAB toolbox
would help position \texttt{pysib} in the wider ecosystem.
