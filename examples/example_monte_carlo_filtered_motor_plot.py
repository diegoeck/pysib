"""Monte-Carlo plots (motor system): load results from mc_results_motor.npz and plot."""
import numpy as np
...
data  = np.load(Path(__file__).parent / "mc_results_motor.npz")
T_oe   = data["T_oe"]
T_filt = data["T_filt"]
B_SIM  = data["B_SIM"]
F_SIM  = data["F_SIM"]
...
fig.suptitle(f"Monte-Carlo motor ({M} runs)")
plt.tight_layout()
plt.show()
...
fig2.suptitle(f"Histogram: OE vs OE filtered — motor ({M} runs)", y=1.02)
plt.tight_layout()
plt.show()
