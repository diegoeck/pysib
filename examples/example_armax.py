"""ARMAX example: estimate y = B/A*u + C/A*e from data."""
import numpy as np
...
# True ARMAX model:  y = u(t-1) / (1-0.9*z^-1) + (1-0.5*z^-1)/(1-0.9*z^-1) * e(t)
...
plt.ylabel("y(t)")
plt.legend()
plt.title("ARMAX: A=" + str(m["A"].round(3)) + " B=" + str(m["B"].round(3)) + " C=" + str(m["C"].round(3)))
plt.show()
